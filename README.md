...
서비스 전체 기획과 핵심 화면 설계를 직접 주도했고,

AI 면접 파트(텍스트/음성) 전체를 맡아 GPT·Whisper 연동, 면접 이력 관리, 피드백 자동화, 마이페이지 연결까지 “실제 서비스 구조”로 구현

자소서/이력서 AI 피드백 기능을 새롭게 설계·구현, 항목별 분석·수정제안 자동화

면접관 등록/목록/상세/예약/리뷰 등 CRUD 전반과 메인 화면 UI/UX 구현

팀원들과의 협업 및 최종 발표(기획→테스트→발표자료)까지 프로젝트 전과정 리딩
